...
import com.example.dmv2.dealmedanv2final.rest.DealItemsResult;
import com.example.dmv2.dealmedanv2final.rest.GetDealItems;
...
//        }
    ...
    public static void getDealitemsRetrofit() {

        String APIURL = "http://103.28.22.89/~kabarcom/api_mobile/";
        Retrofit retrofit = new Retrofit.Builder().baseUrl(APIURL).
                addConverterFactory(GsonConverterFactory.create()).
                build();
        GetDealItems getdealitems = retrofit.create(GetDealItems.class);
        Call<DealItemsResult> call = getdealitems.getDealitems();

//        Response<DealItemsResult> response = null;
//        try {
...
//        }
        ...
        call.enqueue(new Callback<DealItemsResult>() {


            public Context context;

            @Override
            public void onResponse(Call<DealItemsResult> call, Response<DealItemsResult> response) {
                DealItemsResult dealItemsResult=response.body();
                for(com.example.dmv2.dealmedanv2final.rest.Dealitem item : dealItemsResult.getDealitems()) {
                    SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
                    Date dstart = null;
                    try {
                        dstart = format.parse(item.getDateStart());
                    } catch (ParseException e) {
                        e.printStackTrace();
                    }
                    Date dexpired = null;
                    try {
                        dexpired = format.parse(item.getDateExpired());
                    } catch (ParseException e) {
                        e.printStackTrace();
                    }

                    java.sql.Date ds = new java.sql.Date(dstart.getTime());
                    java.sql.Date de = new java.sql.Date(dexpired.getTime());


                    Dealitem.dealitems.add(new Dealitem(
                            item.getNama(),
                            item.getTempat(),
                            Double.parseDouble(item.getHarga()),
                            Integer.parseInt(item.getDiskon()),
                            item.getImage(),
                            Integer.parseInt(item.getStock()),
                            item.getCode(),
                            ds,
                            de,
                            item.getInfo(),
            ...
            @Override
            public void onFailure(Call<DealItemsResult> call, Throwable t) {
                //Handle failure
            }
        });
//        DealItemsResult dealitemsresult = response.body();
